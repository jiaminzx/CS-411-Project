...
def showMatches():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # pyso_view = "CREATE OR REPLACE VIEW pyso_view AS select YT2.viewed__id from yeses_tbl as YT2 where YT2.prospecting_id = {}".format(userID)
            # cursor.execute(pyso_view)
            # print("created pyso_view")
            ...
            # matches_view = "CREATE OR REPLACE VIEW matches_view AS SELECT YT1.prospecting_id FROM yeses_tbl AS YT1 WHERE YT1.viewed__id = {} and YT1.prospecting_id IN pyso_view".format(userID)
            # cursor.execute(matches_view)
            # print("created matches_view")
            # cursor.execute("SELECT * from users WHERE userID IN matches_view")
            # print("queried for match profiles")
            # rows = cursor.fetchall()
            # print("got rows")
